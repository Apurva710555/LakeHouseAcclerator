...
# def create_all_purpose_cluster(application_name, policy_id, node_type_id, base_workers, environment, project_tags):
...
#     if cluster_id:
#         print(f"All-purpose cluster '{cluster_name}' already exists. Using existing ID: {cluster_id}")
#         return cluster_id
...
#     api_response = _safe_request("POST", url, headers=_auth_headers(), json=payload, timeout=FAST_TIMEOUT_SECS, retries=FAST_RETRIES)
...
#         else:
#             raise RuntimeError(f"Cluster creation succeeded but ID was missing from response body: {api_response.get('body')}")
#     else:
#         error_msg = api_response.get("error") or str(api_response.get("body"))
#         raise RuntimeError(f"Failed to create cluster '{cluster_name}'. API Error: {error_msg}")
...
def create_all_purpose_cluster(
    ...
    headers = _auth_headers()
    url = _ws_url("/api/2.0/clusters/create")
    ...
    payload = {
        ...
        "autotermination_minutes": 10,
        "autoscale": {
            "min_workers": max(1, int(base_workers or 1)),
            "max_workers": max(2, int(base_workers or 1) * 2),
        },
        "custom_tags": project_tags,
        "data_security_mode": "USER_ISOLATION",
        "policy_id": policy_id,
    ...
    raise RuntimeError(f"Cluster creation failed: {resp.text}")
...
# --------------------------------------------------------------------------------------------
#  ( Retrieves all Unity Catalog catalogs )  :- added by roshan DQM
# ---------------------------------------------------------------------------------------------
def get_all_catalogs():
    """Retrieves all Unity Catalog catalogs."""

    url = _ws_url("/api/2.1/unity-catalog/catalogs")
    ...
    if status_code != 200:
        error_msg = api_response.get("error") or str(response_body)
        raise RuntimeError(f"Failed to retrieve catalogs. API Error: {error_msg}")

    catalogs = response_body.get("catalogs", [])

    return [c.get("name") for c in catalogs if c.get("name")]


# --------------------------------------------------------------------------------------------
#  ( Retrieves schemas for a given catalog )  :- added by roshan DQM
# ---------------------------------------------------------------------------------------------
def get_schemas_for_catalog(catalog_name: str):
    """Retrieves schemas for a given catalog."""

    url = _ws_url(f"/api/2.1/unity-catalog/schemas?catalog_name={catalog_name}")
    ...
    if status_code != 200:
        error_msg = api_response.get("error") or str(response_body)
        raise RuntimeError(f"Failed to retrieve schemas. API Error: {error_msg}")

    schemas = response_body.get("schemas", [])

    return [s.get("name") for s in schemas if s.get("name")]


# --------------------------------------------------------------------------------------------
# Retrieve all tables for a given catalog and schema from Unity Catalog  :- added by roshan DQM
# --------------------------------------------------------------------------------------------
def get_tables_for_schema(catalog_name: str, schema_name: str):
    ...
    if status_code != 200:
        error_msg = api_response.get("error") or str(response_body)
        raise RuntimeError(f"Failed to retrieve tables. API Error: {error_msg}")
...
def describe_table_schema(full_table_name: str):
    """
    Fetch table schema using Unity Catalog API.
    No SQL warehouse required.
    """

    url = _ws_url(f"/api/2.1/unity-catalog/tables/{full_table_name}")
    ...
    if api_response.get("status_code") != 200:
        raise RuntimeError("Failed to fetch table metadata")

    table_info = api_response.get("body", {})
...
# ----------- End here get_all_catalogs/get_schemas_for_catalog/get_tables_for_schema/describe_table_schema
